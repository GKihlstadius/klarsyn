...
// Sakerstall schemat en gang per instans (funkar aven pa serverless cold start).
let schemaReady
app.use(async (req, res, next) => {
  try {
    schemaReady = schemaReady || ensureSchema()
    await schemaReady
    next()
  } catch (err) {
    schemaReady = null
    res.status(500).json({ error: 'Databasfel', detail: String(err?.message || err) })
  }
})
